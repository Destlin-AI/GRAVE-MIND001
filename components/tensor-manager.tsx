    ...
    <Card className="w-full">
          ...
          <TabsContent value="status" className="space-y-4 mt-4">
            <div className="grid gap-4">
              <div className="flex items-center justify-between">
                ...
                <Switch checked={status.isRunning} onCheckedChange={toggleTensorSystem} disabled={isLoading} />
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex space-x-2">
                  ...
                  />
                  <Button onClick={handleNvmePathSave} disabled={status.isRunning}>
                    Save
                  </Button>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm">{status.allocation.ram}</span>
                </div>
                <Progress
                  value={status.allocation.total > 0 ? (status.allocation.ram / status.allocation.total) * 100 : 0}
                  className="h-2"
                />
              ...
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm">{status.allocation.nvme}</span>
                </div>
                <Progress
                  value={status.allocation.total > 0 ? (status.allocation.nvme / status.allocation.total) * 100 : 0}
                  className="h-2"
                />
              ...
              <div className="space-y-4">
                <div>
                  ...
                  {status.hardware.gpu.available ? (
                    <div className="space-y-2">
                      <div className="text-sm">Count: {status.hardware.gpu.count}</div>
                      {status.hardware.gpu.devices.map((gpu, index) => (
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span>Usage: {status.hardware.cpu.usage.toFixed(1)}%</span>
                      <span>
                        Cores: {status.hardware.cpu.cores} / Threads: {status.hardware.cpu.threads}
                      </span>
                    </div>
                    <Progress value={status.hardware.cpu.usage} className="h-2" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs">
                      <span>
                        Usage: {(status.hardware.ram.totalGB - status.hardware.ram.freeGB).toFixed(1)}GB /
                        {status.hardware.ram.totalGB.toFixed(1)}GB
                      </span>
                      <span>{status.hardware.ram.usagePercent.toFixed(1)}%</span>
                    </div>
                    <Progress value={status.hardware.ram.usagePercent} className="h-2" />
                  </div>
                </div>

                <div>
                  <h4 className="text-sm font-medium mb-2">NVME Storage</h4>
                  {status.hardware.nvme.available ? (
                    <div className="space-y-1">
                      <div className="flex justify-between text-xs">
                        <span>
                          Usage: {(status.hardware.nvme.totalGB - status.hardware.nvme.freeGB).toFixed(1)}GB /
                          {status.hardware.nvme.totalGB.toFixed(1)}GB
                        </span>
                        <span>{status.hardware.nvme.usagePercent.toFixed(1)}%</span>
                      </div>
                      <Progress value={status.hardware.nvme.usagePercent} className="h-2" />
                      <div className="text-xs mt-1">
                        Path: {nvmePath || "Not set"}
                        {status.hardware.nvme.pathExists ? (
          ...
          <TabsContent value="test" className="mt-4">
            <div className="space-y-4">
              ...
              <Button
                ...
                disabled={!status.isRunning || !prompt.trim() || inferenceLoading}
                className="w-full"
              >
                {inferenceLoading ? (
                  <>
      ...
      <CardFooter className="flex justify-between border-t pt-4">
        <div className="text-xs text-muted-foreground">
          {status.isRunning
            ? `Running ${modelName} with ${status.allocation.gpu} GPU, ${status.allocation.cpu} CPU, ${status.allocation.ram} RAM, and ${status.allocation.nvme} NVME layers`
